...
export default {
  ...
  finalize() {
    ...
    Array.from(galleries).forEach((galleryEl) => {
      ...
      const form = document.querySelectorAll("form.variations_form");
      Array.from(form).forEach((form) => {
        //hvae to use pooquery here because jquery uses it and the events are not compatible
        $(form).on("found_variation", (e, variation) => {
          ...
          const slideIndex = Array.from(slides).findIndex(
            (slide) =>
              slide.dataset.thumb === variation.image.gallery_thumbnail_src
          );
